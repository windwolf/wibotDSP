...
template <typename T> struct HysteresisComparerBounds {
  T low_bound;
  T high_bound;
};
 ...
 * @tparam T
 */
template <typename T> class HysteresisComparer {
public:
  /**
   *
   ...
   * @param bounds_count
   */
  HysteresisComparer(const HysteresisComparerBounds<T> bounds[],
                     uint32_t bounds_count)
      : bounds_(bounds), bounds_count_(bounds_count), last_result_(0) {}

  /**
   * @brief
   * 当值处于阈值的滞回区时，返回上一次的结果；否则返回该阈值对的索引，其中0表示最低阈值以下，N表示最高阈值的以上。
   * @param value
   * @return
   */
  uint32_t compare(T value) {
    for (uint32_t i = 0; i < bounds_count_; ++i) {
      if (value < bounds_[i].low_bound) {
        last_result_ = i;
        return last_result_;
      } else if (value >= bounds_[i].low_bound &&
                 value < bounds_[i].high_bound) {
        if (last_result_ <= i) {
          last_result_ = i;
        } else {
          last_result_ = i + 1;
